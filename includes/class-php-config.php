...
require_once ALMGR_PLUGIN_DIR . 'includes/class-wp-config-integration.php';

class ALMGR_PHP_Config {
    ...
    private function load_presets() {
        try {
            $presets_file = ALMGR_PLUGIN_DIR . 'data/presets/php-config.json';
    ...
    public function apply_preset($preset_name) {
        try {
            ...
            if ($this->try_apply_via_wp_config_with_testing($preset['settings'], $original_values)) {
                almgr_config_log('Successfully applied via wp-config.php');
                return true;
            }

            $php_ini_path = ABSPATH . 'php.ini';
            if (almgr_is_file_writable($php_ini_path)) {
                if ($this->apply_via_php_ini($preset['settings'])) {
                    almgr_config_log('Successfully applied via php.ini');
                    return true;
                }
            }

            $htaccess_path = almgr_get_htaccess_path();
            if ($htaccess_path && almgr_is_file_writable($htaccess_path)) {
                $server_type = $this->detect_server_type();
                if ($server_type === 'apache') {
                    if ($this->try_apply_via_htaccess_with_testing($preset['settings'], $original_values)) {
                        almgr_config_log('Successfully applied via .htaccess');
                        return true;
                    }
                }
            }
    ...
    private function try_apply_via_htaccess_with_testing($settings, $original_values) {
        ...
        try {
            $htaccess_service = new ALMGR_Htaccess();
    ...
    private function try_apply_via_wp_config_with_testing($settings, $original_values) {
        ...
        if (!$wp_config_path || !almgr_is_file_writable($wp_config_path)) {
            return false;
        }
    ...
    private function get_fallback_site_url() {
        try {
            ...
            return defined('WP_HOME') ? constant('WP_HOME') : 'https://example.com';
        }
    }
    ...
    private function fallback_http_request($url, $timeout = 5) {
        try {
            ...
            curl_setopt($ch, CURLOPT_USERAGENT, 'Advance Log Manager Config Test');
    ...
    private function test_site_accessibility() {
        try {
            ...
            foreach ($test_endpoints as $test_url) {
                ...
                while ($attempts < $max_attempts && !$success) {
                    ...
                    $response = wp_remote_get($test_url, [
                        'timeout' => $timeout,
                        'sslverify' => false,
                        'user-agent' => 'Advance Log Manager Config Test',
                        'headers' => [
                    ...
                    if (!is_wp_error($response)) {
                        ...
                        if ($status_code >= 200 && $status_code < 400) {
                            $success = true;
                            $successful_tests++;
                            almgr_config_log("{$test_url} responded with {$status_code} (attempt " . ($attempts + 1) . ")");
                            break;
                        } else {
                            almgr_config_log("{$test_url} returned {$status_code} (attempt " . ($attempts + 1) . ")");
                        }
                    } else {
                        ...
                        almgr_config_log("{$test_url} failed - " . $error_msg . " (attempt " . ($attempts + 1) . ")");
                    }
            ...
            if ($is_accessible) {
                almgr_config_log("Site accessibility confirmed - {$successful_tests}/{$total_tests} endpoints accessible");
            } else {
                almgr_config_log("Site accessibility FAILED - only {$successful_tests}/{$total_tests} endpoints accessible (required: {$required_success})");
            }
    ...
    private function apply_fatal_error_handler_changes_safely_simple($settings, $original_values) {
        ...
        try {
            ...
            if (!$this->validate_wp_config_syntax($wp_config_path)) {
                file_put_contents($wp_config_path, $original_content);
                wp_delete_file($emergency_backup);
                return false;
            }
    ...
    private function apply_fatal_error_handler_changes_safely($settings) {
        ...
        if (!$backup_path) {
            return array(
                'success' => false,
                'message' => function_exists('__') ? __('Failed to create backup for fatal error handler changes.', 'advanced-log-manager') : 'Failed to create backup for fatal error handler changes.'
            );
        }

        // Skip initial accessibility test - using WPConfigTransformer for safe editing
        almgr_config_log(' Fail-safe disabled for fatal error handler changes - using WPConfigTransformer');
        ...
        if (!$this->validate_wp_config_syntax($wp_config_path)) {
            almgr_config_log(' Syntax validation failed after fatal error handler changes - reverting');
            $this->restore_backup($wp_config_path, $backup_path);
            wp_delete_file($emergency_backup);
            return array(
                'success' => false,
                'message' => function_exists('__') ? __('Fatal error handler changes reverted: syntax validation failed.', 'advanced-log-manager') : 'Fatal error handler changes reverted: syntax validation failed.'
            );
        }

        // Skip accessibility tests - using WPConfigTransformer for safe editing
        almgr_config_log(' Accessibility tests disabled - relying on WPConfigTransformer safety');

        // Step 5: Final validation
        if (!$this->validate_wordpress_constants($settings)) {
            almgr_config_log(' WordPress constants validation failed after fatal error handler changes - reverting');
            $this->restore_backup($wp_config_path, $backup_path);
            wp_delete_file($emergency_backup);
            return array(
                'success' => false,
                'message' => function_exists('__') ? __('Fatal error handler changes reverted: constants validation failed.', 'advanced-log-manager') : 'Fatal error handler changes reverted: constants validation failed.'
            );
        }
        ...
        almgr_config_log(' Fatal error handler changes applied successfully with enhanced protection');
        return array(
            'success' => true,
            'message' => function_exists('__') ? __('Fatal error handler changes applied successfully with enhanced protection.', 'advanced-log-manager') : 'Fatal error handler changes applied successfully with enhanced protection.'
        );
    }
    ...
    private function apply_via_apache_htaccess($settings) {
        ...
        almgr_config_log(" Writing to .htaccess:\n" . $php_block);
    ...
    private function apply_via_wp_config_constants_only($settings) {
        ...
        return ALMGR_WP_Config_Integration::apply_php_config_safe($settings);
    }
    ...
    private function validate_config_changes($original_values, $target_settings) {
        ...
        foreach ($target_settings as $setting => $target_value) {
            ...
            if (isset($original_values[$setting]) && isset($new_values[$setting])) {
                ...
                if ($new === $target || ($new !== $original && $new >= $target * 0.8)) {
                    $changes_detected++;
                    almgr_config_log(" {$setting} changed from {$original} to {$new} (target: {$target})");
                } else {
                    almgr_config_log(" {$setting} unchanged: {$original} -> {$new} (target: {$target})");
                }
            }
        }
        ...
        if ($success_rate >= 0.5) {
            almgr_config_log(" Validation passed - {$changes_detected}/{$total_settings} settings changed");
            return true;
        } else {
            almgr_config_log(" Validation failed - only {$changes_detected}/{$total_settings} settings changed");
    ...
    private function validate_htaccess_configuration_applied($target_settings) {
        ...
        if (strpos($htaccess_content, 'BEGIN Advance Log Manager PHP Configuration') === false) {
            return false;
        }
        ...
        if ($success) {
            almgr_config_log(" .htaccess validation passed - {$settings_found}/{$total_settings} applicable settings found in file");
        } else {
            almgr_config_log(" .htaccess validation failed - only {$settings_found}/{$total_settings} applicable settings found (needed: {$required_found})");
        }
    ...
    public function detect_configuration_method() {
        ...
        $htaccess_path = almgr_get_htaccess_path();
        if (almgr_is_file_writable($htaccess_path) && function_exists('apache_get_modules')) {
    ...
    private function apply_via_wp_config($settings) {
        ...
        $result = ALMGR_WP_Config_Integration::apply_php_config_safe($settings);
    ...
    private function generate_htaccess_php_block($settings) {
        $block = "# BEGIN Advance Log Manager PHP Config\n";
        ...
        $block .= "# END Advance Log Manager PHP Config";
    ...
    private function generate_wp_config_constants_block($settings) {
        $php_block = "/* BEGIN Advance Log Manager PHP Configuration */\n";
        ...
        $php_block .= "/* END Advance Log Manager PHP Configuration */\n";
    ...
    private function setting_to_wp_constant($setting) {
        $mapping = [
            ...
            'max_input_time' => 'ALMGR_MAX_INPUT_TIME',
            'upload_max_filesize' => 'ALMGR_UPLOAD_MAX_FILESIZE',
            'post_max_size' => 'ALMGR_POST_MAX_SIZE',
            'max_input_vars' => 'ALMGR_MAX_INPUT_VARS'
        ];
    ...
    private function generate_php_ini_content($settings) {
        $content = "; Advance Log Manager PHP Config\n";
    ...
    private function remove_php_config_block($content) {
        $pattern = '/# BEGIN Advance Log Manager PHP Config.*?# END Advance Log Manager PHP Config/s';
    ...
    private function remove_wp_config_php_block($content) {
        ...
        $patterns = [
            ...
            '/\/\*\s*BEGIN\s+ALMGR\s+Toolkit\s+PHP\s+Configuration\s*-\s*WordPress\s+Constants\s+Only\s*\*\/.*?\/\*\s*END\s+ALMGR\s+Toolkit\s+PHP\s+Configuration\s*\*\//s',
            // Match blocks with "Safe WordPress Implementation" string
            '/\/\*\s*BEGIN\s+ALMGR\s+Toolkit\s+PHP\s+Configuration\s*-\s*Safe\s+WordPress\s+Implementation\s*\*\/.*?\/\*\s*END\s+ALMGR\s+Toolkit\s+PHP\s+Configuration\s*\*\//s',
            // Match exactly our /* style PHP config blocks (short version) */
            '/\/\*\s*BEGIN\s+ALMGR\s+Toolkit\s+PHP\s+Config\s*-\s*WordPress\s+Constants\s+Only\s*\*\/.*?\/\*\s*END\s+ALMGR\s+Toolkit\s+PHP\s+Config\s*\*\//s',
            // Match standard PHP config blocks
            '/\/\*\s*BEGIN\s+ALMGR\s+Toolkit\s+PHP\s+Configuration\s*\*\/.*?\/\*\s*END\s+ALMGR\s+Toolkit\s+PHP\s+Configuration\s*\*\//s',
            '/\/\*\s*BEGIN\s+ALMGR\s+Toolkit\s+PHP\s+Config\s*\*\/.*?\/\*\s*END\s+ALMGR\s+Toolkit\s+PHP\s+Config\s*\*\//s',
            // Match our // style comments (legacy)
            '/\/\/\s*BEGIN\s+ALMGR\s+Toolkit\s+PHP\s+Config.*?define\s*\(.*?\/\/\s*END\s+ALMGR\s+Toolkit\s+PHP\s+Config/s'
        ];
        ...
        foreach ($patterns as $pattern) {
            ...
            if ($matches_count > 0 && $reduction_percentage < 50) {
                $content = $new_content;
                almgr_config_log(" Removed " . $matches_count . " config blocks (reduced by " . round($reduction_percentage, 1) . "%)");
            } elseif ($matches_count > 0 && $reduction_percentage >= 50) {
                almgr_config_log(" Regex pattern removed too much content (" . round($reduction_percentage, 1) . "%), reverting");
                break;
            }
        }
    ...
    public function get_config_method_info() {
        ...
        $htaccess_path = almgr_get_htaccess_path();
        if (almgr_is_file_writable($htaccess_path)) {
    ...
    private function register_backup($backup_info) {
        $registry = function_exists('get_option') ? get_option('almgr_php_config_backup_registry', array()) : array();
        ...
        if (function_exists('update_option')) {
            update_option('almgr_php_config_backup_registry', $registry);
        }
    }
    ...
    private function validate_php_syntax_string($php_code) {
        ...
        $temp_file = tempnam(sys_get_temp_dir(), 'almgr_syntax_check_');
        if (!$temp_file) {
            return false;
        }
