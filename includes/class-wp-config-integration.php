...
require_once ALMGR_PLUGIN_DIR . 'includes/WPConfigTransformer.php';

class ALMGR_WP_Config_Integration {
    ...
    public static function apply_php_config_safe($settings) {
        ...
        if (!$wp_config_path || !file_exists($wp_config_path)) {
            almgr_config_log(' wp-config.php not found');
            return false;
        }
        ...
        if (!$backup_path) {
            almgr_config_log(' Failed to create backup');
            return false;
        }
        ...
        try {
            ...
            if (self::validate_wp_config($wp_config_path)) {
                almgr_config_log(' Configuration applied successfully');
                return true;
            } else {
                // Restore backup on validation failure
                copy($backup_path, $wp_config_path);
                almgr_config_log(' Validation failed, backup restored');
                return false;
            }
    ...
    public static function apply_debug_constants_enhanced($debug_settings, $use_custom_log_path = true) {
        ...
        try {
            ...
            if ($use_custom_log_path && isset($debug_settings['WP_DEBUG_LOG']) && $debug_settings['WP_DEBUG_LOG']) {
                ...
                almgr_config_log(' Using custom debug log path: ' . $custom_path);
            }
    ...
    private static function apply_ini_set_safe($transformer, $setting, $value) {
        ...
        if ($safe_setting !== $setting) {
            almgr_config_log(" Invalid setting name: $setting");
            return;
        }
        ...
        $almgr_start_marker = '/* BEGIN Advanced Log Manager PHP Configuration */';
        $almgr_end_marker = '/* END Advanced Log Manager PHP Configuration */';

        // Check if MT block exists
        if (strpos($content, $almgr_start_marker) !== false) {
            // Update existing block
            $pattern = '/\/\* BEGIN Advanced Log Manager PHP Configuration \*\/.*?\/\* END Advanced Log Manager PHP Configuration \*\//s';
            ...
            // Rebuild the block
            $new_block = $almgr_start_marker . "\n";
            foreach ($existing_statements as $statement) {
                $new_block .= " $statement\n";
            }
            $new_block .= " $almgr_end_marker";
    ...
    private static function generate_custom_debug_log_path() {
        ...
        // Use wp-content/advanced-log-manager directory as specified in requirements
    ...
    private static function get_or_create_query_log_path() {
        ...
        if ($wp_config_path && file_exists($wp_config_path)) {
            ...
            $patterns = [
                // Standard pattern: define( 'ALMGR_QUERY_LOG', '/path/to/file' )
                '/define\s*\(\s*[\'"]ALMGR_QUERY_LOG[\'"]\s*,\s*[\'"]([^\'"])+[\'"]\s*\)/',
                // Handle escaped quotes: define( 'ALMGR_QUERY_LOG', '\'''/path/to/file'\''' )
                '/define\s*\(\s*[\'"]ALMGR_QUERY_LOG[\'"]\s*,\s*[\'"]\\\\?[\'"]([^\\\\]+)\\\\?[\'"][\'"] *\)/',
                // Handle double quotes with single quotes: define( "ALMGR_QUERY_LOG", '/path/to/file' )
                '/define\s*\(\s*"ALMGR_QUERY_LOG"\s*,\s*[\'"]([^\'"])+[\'"]\s*\)/',
            ];
    ...
    public static function apply_custom_query_log_path($enable_custom_path = true) {
        ...
        try {
            ...
            if ($enable_custom_path) {
                ...
                if ($transformer->exists('constant', 'ALMGR_QUERY_LOG')) {
                    $transformer->update('constant', 'ALMGR_QUERY_LOG', $formatted_value, [
                    ...
                    $transformer->add('constant', 'ALMGR_QUERY_LOG', $formatted_value, $add_options);
                }

                almgr_config_log(' Applied custom query log path: ' . $custom_path);
            } else {
                // Remove custom path constant
                if ($transformer->exists('constant', 'ALMGR_QUERY_LOG')) {
                    $transformer->remove('constant', 'ALMGR_QUERY_LOG');
                }
            }
    ...
    private static function remove_existing_mt_config($transformer) {
        ...
        foreach ($almgr_constants as $constant) {
            if ($transformer->exists('constant', $constant)) {
        ...
        $pattern = '/\s*\/\* BEGIN Advanced Log Manager PHP Configuration \*\/.*?\/\* END Advanced Log Manager PHP Configuration \*\/\s*/s';
    ...
    private static function create_backup($wp_config_path) {
        $backup_dir = ALMGR_PLUGIN_DIR . 'backups/wp-config/';
    ...
    public static function validate_wp_config($wp_config_path) {
        ...
        if (function_exists('shell_exec') && !empty(shell_exec('which php'))) {
            ...
            if (strpos($output, 'No syntax errors detected') === false) {
                almgr_config_log(' PHP syntax error detected');
                return false;
            }
        }
        ...
        if (strpos($content, '<?php') === false || strpos($content, 'wp-settings.php') === false) {
            almgr_config_log(' wp-config.php structure invalid');
            return false;
        }
        ...
        foreach ($syntax_issues as $pattern => $description) {
            if (preg_match($pattern, $content)) {
                almgr_config_log(" Syntax issue - $description");
                return false;
            }
        }
